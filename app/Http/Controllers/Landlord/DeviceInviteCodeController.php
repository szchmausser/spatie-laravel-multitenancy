...
{
    /**
     * List all invite codes with device and creator info.
     */
    public function index(): Response
    {
        $codes = DeviceInviteCode::query()
            ->with(['device:id,name', 'createdBy:id,name'])
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $expiresAt = ($validated['expires_days'] ?? 0) > 0
        ...
        return redirect()->route('landlord.invite-codes.index')
            ->with('flash', [
                'success' => "Código {$code->code} generado correctamente.",
            ]);
    }
    ...
    {
        $device_invite_code->load(['device:id,name']);
