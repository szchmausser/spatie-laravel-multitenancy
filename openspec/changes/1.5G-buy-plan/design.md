...
- Make the plan change atomic, immediate, and concurrency-safe: `DB::transaction` + `lockForUpdate()`, `ends_at` reset to `now()->addMonth()`, `trial_ends_at` untouched.
...
Authorization diverges at the controller level (permission-based vs. identity-based on the `Landlord` model). The mutation converges in a single service so the write — and the locking semantics that protect it — has exactly one implementation.
...
| `app/Services/Billing/ChangePlanService.php` | NEW Service | `applyPlanChange(Subscription, Plan): void` — `DB::transaction` + `Subscription::lockForUpdate()` + same-plan `abort_if` + `plan_id`/`ends_at` update. First file under `app/Services/`; establishes the "shared mutations in a service" convention. |
...
The service holds the row lock for the duration of the transaction; the second concurrent request blocks at `lockForUpdate()` until the first commits, then re-reads and trips the same-plan guard.
...
1. **`ChangePlanService` skeleton + same-plan guard.** Add `tests/Feature/Billing/ChangePlanControllerTest.php::test_post_with_current_plan_returns_422` first. Then add `App\Services\Billing\ChangePlanService` with `applyPlanChange()` and the same-plan `abort_if` inside `DB::transaction`. Test goes green by calling the service directly (no controller yet). Pins the lock + guard contract before the controller exists.
...
- **`lockForUpdate()` over optimistic locking.** Pessimistic locking keeps the change in a single transaction and the same-plan guard runs against the locked row. The alternative — a `version` column and a 409 retry path — would force the frontend to handle the 409 (extra state machine, extra tests) and would not improve correctness, only contention. The lock is held for the duration of one `update()` call.
...
- **Stale permission cache on tenant switch** (Med, already mitigated). `TenantPermissionsSeeder` already calls `forgetCachedPermissions()`; the row lock in `ChangePlanService` is the second defence. No new mitigation in this slice.
...
- **`Subscription::on('landlord')` resolution in the service** (Info). The `Subscription` model uses `UsesLandlordConnection`, so the same instance is readable from both tenant and landlord contexts. The service does not call `->on('landlord')` explicitly — the model does that itself. The same-plan test in step 1 implicitly proves the lock serializes concurrent calls.
